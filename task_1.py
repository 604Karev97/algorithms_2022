...
def check_1(lst_obj):
    ...
    Сложность: O(N).
    """
    lst_to_set = set(lst_obj)  # O(N) зависит от длины lst_obj
...
def check_3(lst_obj):
    ...
    Сложность: O(len(lst_obj))+O(NlogN)+O(N)*O(1)*O(1)+O(1) = O(NlogN)
    """
    lst_copy = list(lst_obj)                 # O(N)  - зависит от длины lst_obj
